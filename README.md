...
You can access the web app at `http://localhost:8080/{quizId}?userId={userId}`. Here, `{quizId}` should be replaced by a valid ID corresponding to an actual quiz creating using the backend and `{userId}` can be any valid user identifier. For now, there are no explicit checks in place for `userId`. So, you can use any valid string.
